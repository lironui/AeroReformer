...
class MMBasicLayer(nn.Module):
    def __init__(self,
        ...
        if fusion == 'PWAM':
            ...
            self.fusion = VLCAM(dim,  # both the visual input and for combining, num of channels
                                dim,  # v_in
                                768,  # l_in
                                dim,  # key
                                dim,  # value
...
class VLCAM(nn.Module):

    def __init__(self, dim, v_in_channels, l_in_channels, key_channels, value_channels, num_heads=8, dropout=0.0):
        super(VLCAM, self).__init__()

        # Project vision features from `dim` to `v_in_channels`.
        self.vis_project = nn.Sequential(
            nn.Conv1d(dim, v_in_channels, kernel_size=1),
            nn.GELU(),
            nn.Dropout(dropout)
        )

        # Cross-attention module: vision features (queries) attend to language features (keys & values).
        self.image_lang_attn = VisionLanguageAttention(
            v_in_channels,  # v_in
            l_in_channels,  # l_in
            key_channels,  # key
            value_channels,  # value
        ...
        self.project_mm = nn.Sequential(
            nn.Conv1d(v_in_channels, v_in_channels, kernel_size=1),
            nn.GELU(),
            nn.Dropout(dropout)
        )
    ...
    def forward(self, x, l, l_mask):
        ...
        x_t = x.permute(0, 2, 1)  # (B, dim, H*W)
        x_proj = self.vis_project(x_t)  # (B, v_in_channels, H*W)

        # l_mask = l_mask.squeeze(-1)
        # # Convert mask to bool: pad positions are where l_mask==0
        # l_mask = (l_mask == 0)

        # Apply cross-attention: vision attends to language.
        attn_out = self.image_lang_attn(x, l, l_mask)

        attn_out = attn_out.permute(0, 2, 1)  # (B, dim, H*W)

        fusion_t = torch.mul(x_proj, attn_out)
        # Final projection.
        fusion_out = self.project_mm(fusion_t)  # (B, v_in_channels, H*W)
        fusion_out = fusion_out.permute(0, 2, 1)  # (B, H*W, v_in_channels)
...
class VisionLanguageAttention(nn.Module):
    def __init__(
        ...
        if out_channels is None:
            out_channels = key_channels  # We'll project from key_channels to out_channels later
        ...
        self.f_key = nn.Conv1d(l_in_channels, key_channels, kernel_size=1)
        self.f_value = nn.Conv1d(l_in_channels, value_channels, kernel_size=1)

        self.f_query = nn.Sequential(
            nn.Conv1d(v_in_channels, key_channels, kernel_size=1),
            nn.InstanceNorm1d(key_channels),
        )

        self.W_o = nn.Sequential(
            nn.Conv1d(key_channels, out_channels, kernel_size=1),
            nn.InstanceNorm1d(out_channels),
        )

        if use_pos_enc:
            # Learnable positional embedding for each spatial token.
            self.pos_embedding = nn.Parameter(torch.randn(1, key_channels, 1))
        else:
            self.pos_embedding = None

        self.mha = nn.MultiheadAttention(
            embed_dim=key_channels,
            num_heads=num_heads,
            kdim=key_channels,
            vdim=value_channels,
            dropout=dropout,
    ...
    def forward(self, v_features, l_features, l_mask=None):
        B, HW, _ = v_features.shape

        if l_features.shape[1] != self.l_in_channels:
            l_features = l_features.permute(0, 2, 1)  # (B, l_in_channels, N_l)

        v_features_1d = v_features.permute(0, 2, 1)
        query = self.f_query(v_features_1d)  # (B, key_channels, HW)
        if self.pos_embedding is not None:
            query = query + self.pos_embedding.expand(-1, -1, HW)
        query = query.permute(0, 2, 1)  # (B, HW, key_channels)

        key = self.f_key(l_features)  # (B, key_channels, N_l)
        value = self.f_value(l_features)  # (B, value_channels, N_l)
        key = key.permute(0, 2, 1)  # (B, N_l, key_channels)
        value = value.permute(0, 2, 1)  # (B, N_l, value_channels)

        key_padding_mask = None
        if l_mask is not None:
            key_padding_mask = (l_mask.squeeze(-1) == 0)

        attn_output, _ = self.mha(
            query, key, value, key_padding_mask=key_padding_mask
        )  # attn_output: (B, HW, key_channels)

        attn_output = attn_output.permute(0, 2, 1)
        # print(attn_output.shape, v_features.shape)
        # attn_output = attn_output + v_features.permute(0, 2, 1)  # ablation
        out_1d = self.W_o(attn_output)  # (B, out_channels, HW)
        out = out_1d.permute(0, 2, 1)  # (B, HW, out_channels)

        out_normed = self.norm_attn(out)  # (B, HW, out_channels)
        if self.ffn is not None:
            # For Conv1d, permute to (B, out_channels, HW)
            ffn_input = out_normed.permute(0, 2, 1)
            ffn_output = self.ffn(ffn_input)
            ffn_output = ffn_output.permute(0, 2, 1)  # (B, HW, out_channels)
            out = out_normed + ffn_output
            out = self.norm_ffn(out)
        else:
            out = out_normed

        if self.out_channels != self.v_in_channels:
            out_proj = out.permute(0, 2, 1)
            proj_layer = nn.Conv1d(self.out_channels, self.v_in_channels, kernel_size=1).to(out.device)
            out_proj = proj_layer(out_proj)
            out = out_proj.permute(0, 2, 1)  # (B, HW, v_in_channels)
