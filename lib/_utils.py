...
def load_weights(model, load_path):
    dict_trained = torch.load(load_path, weights_only=True)['model']
...
class _LAVTOneSimpleDecode(nn.Module):
    ...
    def forward(self, x, text, l_mask):
        ...
        ### DMMI
        l_feats1 = self.text_encoder(l_mask, attention_mask=l_mask)[0]  # (6, 10, 768)
        l_feats1 = l_feats1.permute(0, 2, 1)  # (B, 768, N_l)
